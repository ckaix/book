...
```
flag.StringVar(&name, "name", "everyone", "The greeting object.")复制代码
```
...
```
var name = flag.String("name", "everyone", "The greeting object.")复制代码
```
...
```
go run demo2.go -name="Robert" 复制代码
```
...
```
$ go run demo2.go --help复制代码
```

其中的`$`表示我们是在命令提示符后运行`go run`命令的。运行后输出的内容会类似：

```
Usage of /var/folders/ts/7lg_tl_x2gd_k1lm5g_48c7w0000gn/T/go-build155438482/b001/exe/demo2: -name string    The greeting object. (default "everyone")exit status 2复制代码
```

你可能不明白下面这段输出代码的意思。

```
/var/folders/ts/7lg_tl_x2gd_k1lm5g_48c7w0000gn/T/go-build155438482/b001/exe/demo2复制代码
```
...
```
$ go build demo2.go$ ./demo2 --help复制代码
```

那么输出就会是

```
Usage of ./demo2: -name string    The greeting object. (default "everyone")复制代码
```
...
```
flag.Usage = func() { fmt.Fprintf(os.Stderr, "Usage of %s:\n", "question") flag.PrintDefaults()}复制代码
```

那么当运行

```
$ go run demo3.go --help复制代码
```

后，就会看到

```
Usage of question: -name string    The greeting object. (default "everyone")exit status 2复制代码
```
...
```
flag.CommandLine = flag.NewFlagSet("", flag.PanicOnError)复制代码
```
...
```
var cmdLine = flag.NewFlagSet("question", flag.ExitOnError)复制代码
```
...
别担心，我在后面会经常提到它的。另外，如果你想详细了解`flag`包的用法，可以到[这个网址](https://golang.google.cn/pkg/flag/)查看文档。或者直接使用`godoc`命令在本地启动一个 Go 语言文档服务器。怎样使用`godoc`命令？你可以参看[这里](https://github.com/hyper0x/go_command_tutorial/blob/master/0.5.md)。
