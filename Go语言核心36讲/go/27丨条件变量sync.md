...
```
var mailbox uint8var lock sync.RWMutexsendCond := sync.NewCond(&lock)recvCond := sync.NewCond(lock.RLocker())复制代码
```
...
```
lock.Lock()for mailbox == 1 { sendCond.Wait()}mailbox = 1lock.Unlock()recvCond.Signal()复制代码
```
...
```
lock.RLock()for mailbox == 0 { recvCond.Wait()}mailbox = 0lock.RUnlock()sendCond.Signal()复制代码
```
