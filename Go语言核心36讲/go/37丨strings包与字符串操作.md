...
**先来说说`string`类型。** 我们都知道，在 Go 语言中，`string`类型的值是不可变的。 如果我们想获得一个不一样的字符串，那么就只能基于原字符串进行裁剪、拼接等操作，从而生成一个新的字符串。
...
```
var builder1 strings.Builderbuilder1.Grow(1)builder3 := builder1//builder3.Grow(1) // 这里会引发 panic。_ = builder3复制代码
```
...
```
f2 := func(bp *strings.Builder) { (*bp).Grow(1) // 这里虽然不会引发 panic，但不是并发安全的。 builder4 := *bp //builder4.Grow(1) // 这里会引发 panic。 _ = builder4}f2(&builder1)复制代码
```
...
```
builder1.Reset()builder5 := builder1builder5.Grow(1) // 这里不会引发 panic。复制代码
```
...
```
var reader1 strings.Reader// 省略若干代码。readingIndex := reader1.Size() - int64(reader1.Len()) // 计算出的已读计数。复制代码
```
...
```
`Count`、`IndexRune`、`Map`、`Replace`、`SplitN`、`Trim`，等等。复制代码
```
